...
A Rails 3 plugin that allows one to inherit or override single templates for controller subclasses. Finally, also views and partials remain DRY!
...
To use inheritable templates in a certain controller hierarchy, add this call to the base controller:
 render_inheritable
...
Nothing sophisticated, but it will do to explain our little gem. With the call to _render_inheritable_ in the CrudController, we tell all +render+ calls in this controller, its subclasses and their corresponding views to perform a template lookup. What does this mean? When we call the index action of our MessagesController, no corresponding template is found in _views/messages/index.html.erb_. The lookup proceeds to the superclass controller, where it finds the template in _views/crud/index.html.erb_. The same holds for the two partials _views/crud/_list.html.erb_ and _views/crud/_entry.html.erb_. 

With this mechanism, we are able to define generic templates that are *inherited* to various controllers. There is no need to duplicate templates when they basically look the same.
...
Now, the <code>render :partial => 'entry'</code> call in _views/crud/_list.html.erb_ will find the partial in the _views/messages_ folder and render it. The other two templates do not have to be touched and are still inherited form the _crud_ folder. So it's possible to customize certain HTML snippets by using specifically 'overriden' partials. For this example, you could also create a partial _views/messages/_list.html.erb_ to display a table instead of a list for messages.
